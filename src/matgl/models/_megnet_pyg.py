...
Mirrors :class:`matgl.models._megnet_dgl.MEGNet` using ``edge_index`` /
scatter-based message passing. Designed so a state dict trained against the
DGL implementation can be loaded into this class one-to-one (parameter names
match), enabling cross-backend parity tests.
"""
...
class MEGNet(MatGLModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize MEGNet (PyG).

        Argument semantics match the DGL implementation. See
        :class:`matgl.models._megnet_dgl.MEGNet` for details.
        """
        super().__init__()
