...
Mirrors :class:`matgl.models._m3gnet_dgl.M3GNet` using ``edge_index`` /
scatter-based message passing and a tensor-bundle line graph from
``matgl.graph._compute_pyg.create_line_graph``. Designed so a state dict
trained against the DGL implementation can be loaded into this class one-to-
one (parameter names match), enabling cross-backend parity tests.
"""
...
class M3GNet(MatGLModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the M3GNet (PyG) model.

        Argument semantics match the DGL implementation. See
        :class:`matgl.models._m3gnet_dgl.M3GNet` for full descriptions.
        """
        super().__init__()
    ...
    def forward(
        ...
        Args:
            ...
            l_g: Cached line-graph bundle from
                :func:`matgl.graph._compute_pyg.create_line_graph`. If ``None``,
